    ...
    return (
        ...
        if month in ["September, October, November"]
            ...
            if month in ["December", "January", "February"]
                ...
                else "Summer" if month in ["June", "July", "August"] else "not a month"
            )
        )
    )
...
print(solve_quadratic_eqn(4, 7, 2))
...
print(capitalize_list_items(["peter", "john"]))
...
print(f"The number of odds are {evens_and_odds(100)[1]}.")  # The number of odds are 50.
print(
    f"The number of evens are {evens_and_odds(100)[0]}."
...
def is_prime(number):
    if number < 2:
        return False
    for x in range(int(math.sqrt(number)) + 1)[2:]:
        if number % x == 0:
            return False
    return True
...
def is_distinct_list(lst):
    return len(lst) == len(set(lst))


print(is_distinct_list([2, 5, 7, 8]))
print(is_distinct_list(["a", "d", "g", "d"]))
...
print(same_data_type([2, 5, 7, 8]))
print(same_data_type(["2", 5, True, 8]))
print(same_data_type(["a", "d", "g", "d"]))
...
print(is_valid_variable_name("hello"))  # True: 'hello' is a valid variable name
print(is_valid_variable_name("123var"))  # False: '123var' starts with a digit
...
def most_spoken_languages(countries_data, top_n=10):
    # Flatten the list of languages and count occurrences
    language_count = Counter(
        lang for country in countries_data for lang in country["languages"]
    )
    # Return the top N most common languages
...
def most_populated_countries(countries_data, top_n=10):
    population_sorted = sorted(
        ((country["name"], country["population"]) for country in countries_data),
        key=lambda x: x[1],
        reverse=True,
    )
    return population_sorted[:top_n]
